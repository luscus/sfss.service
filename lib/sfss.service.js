...
var bodyParser = require('body-parser'),
    ...
    api = express(),
    finder = require('package.module.finder'),
    cache = {},
    store;
...
function init (_options) {
  ...
  function parseRequest (request, response, next) {
    ...
    if (request._context.pathArray[0].match(/^v\d{1,3}\.\d{1,3}(\.\d{1,3}){0,1}$/)) {
  ...
  function processHttpMethod (data) {
    ...
    try {
      ...
      if (! cache[endpointName].process) {
        cache[endpointName].process = require(cache[endpointName].package)(cache[endpointName].options);
      }
